...
public class LeadConverterJbpm extends SlingAllMethodsServlet {
	...
	protected void doPost(SlingHttpServletRequest request, SlingHttpServletResponse response)
		...
		if (request.getRequestPathInfo().getExtension().equals("JBPM")) {
		...
		}else if (request.getRequestPathInfo().getExtension().equals("JBPM_NEW")) {
			...
			try {
				...
				String statusjbpm = request.getParameter("Status");
				out.println("statusjbpm : " + statusjbpm);
				String Created_By = request.getParameter("Created_By");
				out.println("Created_By : " + Created_By);
				String list_id_JBPM = request.getParameter("List_Id");// From JBPM
				out.println("list_id_JBPM : " + list_id_JBPM);
				String funnelNodeName = request.getParameter("funnelNodeName");
				out.println("funnelNodeName : " + funnelNodeName);
				String subFunnelNodeName = request.getParameter("subFunnelNodeName");
				...
				//Subscriber_Id   Status  Created_By subFunnelNodeName  funnelNodeName
				
				Node subscriber_email_nodes = session.getRootNode().getNode("content").getNode("LEAD_CONVERTER")
						.getNode("SUBSCRIBER");
				Node subscriber_email_node=null;
				String subscriber_email_node_name=null;
				String subscriber_id_sling=null;
				String status_sling=null;
				if(subscriber_email_nodes.hasNode(subscriber_email.replace("@", "_"))){
					subscriber_email_node = subscriber_email_nodes.getNode(subscriber_email.replace("@", "_"));
					subscriber_email_node_name = subscriber_email_node.getName();
					subscriber_id_sling = subscriber_email_node.getProperty("SUBSCRIBER_ID").getString();
					status_sling = subscriber_email_node.getProperty("Status").getString();
					out.println("subscriber_email_node_name : " + subscriber_email_node_name);
					out.println("subscriber_id_sling : " + subscriber_id_sling);
					out.println("statussling : " + status_sling);
					
					if (status_sling.equals(statusjbpm)) {
						out.println("Equals Status in both");
						NodeIterator mainnode = session.getRootNode().getNode("content").getNode("user")
								.getNode(Created_By).getNode("Lead_Converter").getNode("Email").getNode("Campaign").getNodes();
						Node typenode = null;
				...
				if (subscriberid.equals(subscriber_id_sling)) {
					...
					if (subscriber_email_node.hasNode("Campaign")) {
						campaignnode = subscriberidnode.getNode("Campaign");
					} 
					String slinglist = null;
					...
					while (campaignnodeitr.hasNext()) {
						...
						//91.101.165.251/restapi/list-subscriber/listSubscriberDelete.php?list_id=oldlist&subscriber_id=subscriberidsling
						usernode.remove();
						// out.println("API RESPONSE : :"+list_id_JBPM+slinglist+apiresponse);
				...
				/*
				Typename : : Explore
				Current_Campaign : : viki_gmail.com_Explore_1
				campaignnextnodenode /content/user/viki_gmail.com/Lead_Converter/Email/Campaign/Explore/viki_gmail.com_Explore_1
				currentcampaignnextname : : viki_gmail.com_Explore_1
				If Current Campaign equals
				campaignnextnodenode /content/user/viki_gmail.com/Lead_Converter/Email/Campaign/Explore/viki_gmail.com_Explore_2
				currentcampaignnextname : : viki_gmail.com_Explore_2
				Else Current Campaign
				Typename : : Inform
				Exception : : :Current_Campaign
				*/
				
				/*
				
				NodeIterator subscriber_email_itr = session.getRootNode().getNode("content").getNode("LEAD_CONVERTER")
						.getNode("SUBSCRIBER").getNodes();
				
				while (subscriber_email_itr.hasNext()) {
					Node subscriberemail = subscriber_email_itr.nextNode();
					subscriberemailname = subscriberemail.getName();
					subscriberidsling = subscriberemail.getProperty("SUBSCRIBER_ID").getString();
					out.println("subscriberemailname : " + subscriberemailname);
					out.println("subscriberidsling : " + subscriberidsling);
